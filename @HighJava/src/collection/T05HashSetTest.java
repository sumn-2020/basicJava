...
public class T05HashSetTest {
	public static void main(String[] args) {
		...
		System.out.println("Set 데이터: " + isAdded);
		System.out.println();

		// Set의 데이터를 수정하려면 수정하는 메서드가 따로 없기 때문에 해당 자룔 삭제한 후 새로운 데이터를 추가해 주어야 한다.
		...
		// 데이터 개수만큼 반복하기
		// hasNext() => 포인터를 다음 위치에 데이터가 있으면 true, 없으면 false를 반환함
		while (it.hasNext()) { // 다음에 데이터 있는지 검사
			// next() => 포인터를 다음 자료 위치로 이동하고 이동한 위치의 자료를 반환함
			System.out.println(it.next());
		}

		// hashSet: 1~100사이의 중복되지 않는 정수 5개 만들기
		Set<Integer> intRnd = new HashSet<Integer>();

		while (intRnd.size() < 5) { // 중복되는 수가 출력되면 다섯번보다 더 많이 돔 (왜냐? SEt은 중복을 허용하지 않기 때문에)
