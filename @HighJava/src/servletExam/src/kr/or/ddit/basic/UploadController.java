...
public class UploadController extends HttpServlet {
	// 파일 업로드는 get방식 없음(doGET안써도 됨)

	private static final String UPLOAD_DIR = "uploda_files";
	// 메모리 임계크기(이 크기가 넘어가면 레파지토리 위치에 임시파일로 저장됨)
	private static final int MEMORY_THRESHOLD = 1024 * 1024 * 3; //3메가
	// 파일 1개당 최대크기
	private static final long MAX_FILE_SIZE = 1024 * 1024 * 40; //40메가
	// 요청 파일 최대크기
	private static final long MAX_REQUEST_SIZE = 1024 * 1024 * 50; //50메가 
			...
			try {
				...
				if (itemList != null && itemList.size() > 0) {
					for (FileItem item : itemList) {
						...
						//파일이냐 그냥 데이터냐 (FormField(form데이터), string,...) 
