...
//    	System.out.println("appointmentVOList : " + appointmentVOList);
    ...
    public int insertAppointment(AppointmentController controller) {
    	...
    	String patinfo = patientSession.getpatCode();
    	appointSession.setPatCode(patinfo);
    	String patCode = appointSession.getPatCode();
    ...
    // 삭제
    public int deleteReply(ReplyController controller) {
    	int number;
    	number= HomeMenu.DELETEQNA_ADMIN.getMenu();
    	String code = scanner.next();
    	number = controller.deleteReply(new ReplyVO(code));
    	System.out.println("삭제되었습니다.");
    	number = HomeMenu.QNA_ADMIN.getMenu();
    	return number;
    }
